...
class EntityLinker:
    ...
    def link(self, question_text: str) -> Dict[str, Any]:
        ...
        excluded_cat = None
        excl_patterns = [
            r'(?:excluding|exclude|remove the|set aside|dropping the|after the|filter out|stripped out|strip out)\s+([a-zA-Z\s\&]+?)(?:\s+(?:division|work|segment|scope|assignment|project|line|category|piece)\b|[,\u2014\-;]|\bwhat\b|\bis\b|\bfor\b|\bbefore\b|\bsum\b|\bisn)',
            r'(?:without|minus)\s+(?:the\s+)?([a-zA-Z\s\&]+?)(?:\s+(?:division|work|segment|scope|category)\b|[,\u2014\-;]|\bwhat\b|\bis\b|\bfor\b)',
            r'carrying the\s+([a-zA-Z\s\&]+?)\s+(?:piece|segment|work|division)',
        ]
